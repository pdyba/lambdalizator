...
class Request:
    ...
    def _safe_json_loads(payload: str | bytes) -> Any:
        try:
            ...
            raise BadRequestError("Request body could not be parsed as valid JSON") from error
    ...
    def json_body(self) -> dict | None:
        if self._json_body is None:
            ...
            if content_type is None:  # pylint: disable=consider-using-assignment-expr
            ...
            elif content_type.startswith(ContentType.JSON):
                ...
                raise BadRequestError(f"Content-Type is not JSON-compatible: {content_type}")

...
class TestAuthorizerWithoutMockingJWT:
    def test__init__(self, full_access_auth_header: str) -> None:
    ...
    def test__repr__(self, full_access_auth_header: str) -> None:
        authz = Authorizer(full_access_auth_header, "test_resource", "permission_name")
        assert repr(authz) == (
...
class TestAuthorizerWithMockedJWT:
    ...
    def test_check_allow_all(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
        authz._check_allow_and_set_resources()  # pylint: disable=protected-access
        assert authz.outcome == ALLOW

    def test_check_deny_any(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
        authz.deny = {ALL: ALL}
        with pytest.raises(PermissionDenied):
            authz._check_deny()  # pylint: disable=protected-access
    ...
    def test__check_access__outcome_deny_bcs_deny_all(self, jwt_partial_payload: dict) -> None:
    ...
    def test__check_resource(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
    ...
    def test_validate_allow_all(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
        authz.check_access()
        assert authz.outcome == ALLOW
        assert authz.restrictions == {"allow": "*", "deny": None}

    def test_check_allow_fail_all(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer({**jwt_partial_payload, "allow": {}, "deny": {}})
        with pytest.raises(PermissionDenied):
            authz._check_allow_and_set_resources()  # pylint: disable=protected-access
            assert authz.outcome == ALLOW

    def test_check_allow_resource(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer(
    ...
    def test_check_allow_one(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
    ...
    def test_check_allow_one_scope(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
    ...
    def test_check_allow_fail_one_scope(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
    ...
    def test_get_restrictions_restricted(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
        authz.allowed_resource = ALL
        authz.denied_resource = {"city": "warszawa"}
        assert authz.restrictions == {"allow": ALL, "deny": {"city": "warszawa"}}

    def test_check_deny_one_scope(self, jwt_partial_payload: dict) -> None:
        authz = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
                "allow": {},
    ...
    def test_allow_if_all(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
        assert authz._allow_if_allow_all(ALL)  # pylint: disable=protected-access
        assert authz.outcome == ALLOW
        assert authz.allowed_resource == ALL

    def test_check_deny_one(self, full_access_authz_payload: dict) -> None:
        authz = self._make_mocked_authorizer(full_access_authz_payload)
    ...
    def test_validate_fail_all(self, full_access_authz_payload: dict) -> None:
    ...
    def test_validate_one(self, jwt_partial_payload: dict) -> None:
    ...
    def test_validate_one_scope(self, jwt_partial_payload: dict) -> None:
        authorizer = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
                "allow": {"test_resource": {"permission_name": {"allow": "self"}}},
    ...
    def test_validate_fail_one_scope(self, jwt_partial_payload: dict) -> None:
        authorizer = self._make_mocked_authorizer(
            {
                **jwt_partial_payload,
                "allow": {"test_resource": {"permission_name": {"deny": "self"}}},
    ...
    def test_refs(self, jwt_partial_payload: dict) -> None:

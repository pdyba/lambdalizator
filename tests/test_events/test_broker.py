...
from lbz.events import BaseEventBroker, CognitoEventBroker, Event, EventBroker, EventHandler
...
class TestBaseEventBroker:
    def test_broker_works_properly(self) -> None:
        ...
        mapper: Mapping[str, list[EventHandler]] = {"x": [func_1, func_2]}
    ...
    def test_broker_continues_even_if_one_handler_failed(self, caplog: LogCaptureFixture) -> None:
        ...
        func_3 = MagicMock()
        expected_event = Event({"y": 1}, event_type="x")
        mapper: Mapping[str, list[EventHandler]] = {"x": [func_1, func_2, func_3]}
        event = {"my-type": "x", "data": {"y": 1}}
